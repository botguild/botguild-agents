...
async function main(): Promise<void> {
  ...
  async function executeAcceptedFlow(gig: Gig, contract: Contract): Promise<void> {
    ...
    try {
      ...
      // Deliver Milestone 1 — fetch + validate. Checkpoint delivery is
      // non-fatal: on a recovery re-run the milestone may already be delivered
      // (the platform 404s a re-delivery); that must not abort the pipeline.
      if (m1Id) {
        try {
          await client.deliverMilestone(contractId, m1Id, { note: extractSummary });
        } catch (err) {
          log.warn({ err, milestoneId: m1Id }, 'milestone 1 checkpoint delivery failed');
        }
      }
      ...
      // Deliver Milestone 2 — transform (checkpoint, non-fatal like M1)
      if (m2Id) {
        try {
          await client.deliverMilestone(contractId, m2Id, { note: normalizeStats.summary });
        } catch (err) {
          log.warn({ err, milestoneId: m2Id }, 'milestone 2 checkpoint delivery failed');
        }
      }
  ...
  webhookServer.on('proposal.accepted', async (event) => {
    ...
    // we'd stash (and later run the ETL pipeline against) a sibling's contract.
  ...
  // milestone.funded and final delivery (deploy, crash, OOM) used to strand
  // the job: nothing re-runs the pipeline, and the funded-handler guard
  // ignores webhook replays once the job is past 'awaiting_funding' (platform
  // #301's execution-side twin). Re-fetch each contract and re-run the
  // pipeline if the platform still expects work. This happens BEFORE
  ...
  for (const job of listJobs()) {
    ...
    // ownership guard existed (cross-bot routing).
    if (!isOwnContract(contract, effectiveBotId)) {
      log.info(
        { contractBotId: contract.botId },
    ...
    // The contract is active and ours — we need the gig to plan the work. If
    // the payer deleted it, rebuild a minimal gig from the contract so the
    // pipeline can still finish rather than stranding a funded contract on a 404.
    let gig: Gig;
    try {
