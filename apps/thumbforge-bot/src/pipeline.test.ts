...
const BRIEF =
  ...
  JSON.stringify({
    ...
    social_pack: { copy: ['Buy now', 'Save big'], count: 2, formats: ['feed'] },
  }) +
  '\n```';
...
test('a flagged headline rejects the job before any render or fan-out (FR-14)', async () => {
  const flagged: Moderator = {
    moderate: async (): Promise<ModerationOutcome> => ({ status: 'flagged', reason: 'scam' }),
  };
...
test('moderation unavailable throws (fail closed → queue retry), never delivers unmoderated', async () => {
  ...
  await assert.rejects(
    () => processRenderMessage(cfg, { kind: 'plan', contractId: 'c1', jobKey }),
