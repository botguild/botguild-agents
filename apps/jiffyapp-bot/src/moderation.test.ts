...
import { MODERATION_MODEL, createModerationClient, moderationInputForImage } from './moderation.js';
...
// ---- moderateImage: same fail-closed semantics, multi-modal input shape ----

test('moderationInputForImage builds the omni multi-modal image_url input', () => {
  const input = moderationInputForImage('data:image/png;base64,AAAA');
  assert.deepEqual(input, [
    { type: 'image_url', image_url: { url: 'data:image/png;base64,AAAA' } },
  ]);
});

test('moderateImage posts the multi-modal input and returns a verdict on a clean image', async () => {
  let requestBody: { model?: string; input?: unknown } = {};
  ...
  const client = createModerationClient({
    ...
    fetchImpl: stubFetch((_url, init) => {
      requestBody = JSON.parse(String(init.body)) as typeof requestBody;
      return jsonResponse(200, { model: MODERATION_MODEL, results: [{ flagged: false }] });
    }),
  });

  const outcome = await client.moderateImage(dataUrl);

  assert.equal(requestBody.model, MODERATION_MODEL);
  assert.deepEqual(requestBody.input, moderationInputForImage(dataUrl));
  assert.ok(outcome.ok);
  assert.equal(outcome.verdict.flagged, false);
  assert.equal(outcome.verdict.checkedAt, '2026-07-06T00:00:00.000Z');
});

test('moderateImage: flagged image comes back ok:true with flagged:true', async () => {
  const client = createModerationClient({
    apiKey: 'k',
    fetchImpl: stubFetch(() =>
      jsonResponse(200, { model: MODERATION_MODEL, results: [{ flagged: true }] }),
  ...
  const outcome = await client.moderateImage('data:image/png;base64,AAAA');
  assert.ok(outcome.ok);
  assert.equal(outcome.verdict.flagged, true);
});

test('moderateImage: non-2xx fails CLOSED as an outage', async () => {
  const client = createModerationClient({
    apiKey: 'k',
...
test('moderateImage: network throw fails CLOSED as an outage', async () => {
  const client = createModerationClient({
    apiKey: 'k',
    fetchImpl: (async () => {
      throw new Error('connect timeout');
...
test('moderateImage: missing results[0].flagged fails CLOSED', async () => {
  const client = createModerationClient({
    apiKey: 'k',
    fetchImpl: stubFetch(() => jsonResponse(200, { model: MODERATION_MODEL })),
  });
