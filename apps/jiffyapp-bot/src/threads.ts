// Contract-thread reading (FR-1 pattern, VoiceWright verbatim): post-funding brief
// corrections arrive as buyer thread posts, and AgentClient only writes messages — so
// this module reads them with the same auth + casing conventions (X-API-Key,
// mapKeysToCamel). Polled from the 15-min cron sweep for jobs parked with reason
// 'brief_invalid'.
 ...
 * (e.g. parseJiffyBrief). Messages from this bot are skipped — the correction
