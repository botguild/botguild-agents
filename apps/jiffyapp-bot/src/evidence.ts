...
2. Open \`wrangler.jsonc\` — it contains a commented-out \`account_id\` line. Uncomment and set
   it, or just run \`npx wrangler deploy\` and pick your account when prompted.
 ...
 *  `verifyEjectZip`'s comment-strip) over the minimal JSON body Wrangler needs. Includes
 *  a commented-out account_id placeholder inside the JSON body. */
export function buildEjectWranglerConfig(slug: string): string {
  return `// Self-host config generated by JiffyApp
{
  // "account_id": "YOUR_CLOUDFLARE_ACCOUNT_ID", — uncomment and set, or let wrangler prompt you
  "name": "${slug}",
  "main": "index.mjs",
