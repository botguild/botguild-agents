...
test('validateSlots enforces kinds and function deny-tokens', () => {
  ...
  assert.deepEqual(
    validateSlots(def, {
      headline: 'Hi',
      compute: '(inputs) => ({ isImportant: true, geolocation: inputs.geolocation, fetching: inputs.fetching, prefetchHint: 1 })',
    }),
    [],
  );
  ...
  assert.deepEqual(
    validateSlots(def, { headline: 'Hi', compute: '(inputs) => ({ sessionStorageKeyName: inputs.sessionStorageKeyName })' }),
    [],
  );
  ...
  assert.ok(validateSlots(def, { headline: 'Hi', compute: '() => new Function("return 1")()' }).length > 0);
