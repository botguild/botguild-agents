...
**Relevance is fuzzy, not an exact-category gate.** `scoreRelevance` (in `scorer.ts`) gives the full 40 for an exact `categories` match, but a gig whose category doesn't match still scores partial relevance if its text shares `keywords` with the bot's description — `round(40 × hits / keywordsForFullScore)`, requiring at least `minKeywordHits` (default 2) distinct hits before the fallback awards anything (a single stray keyword is noise, not a near-match — issue #60). This lets a bot bid on *any job near its description*. Below the hit minimum (or with no category match) relevance is 0 and the gig is skipped, so bots never bid on unrelated work. The spec-quality factors (warranty/clarity/timeline) are scaled by `relevance / 40`, so a well-written but barely-relevant gig can't carry the score past the threshold on spec quality alone. Each bot config sets `keywords` + `keywordsForFullScore` and a lowered `proposalThreshold` (~40) so near-description gigs clear the bar.
